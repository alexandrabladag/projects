...
export default function TaskCategories({ categories }) {
    ...
    const remove = (c) => {
        const extra = c.task_count > 0 ? ` ${c.task_count} task(s) using it will move to "General".` : '';
        if (confirm(`Delete "${c.name}"?${extra}`)) router.delete(route('categories.destroy', c.id));
    };
        ...
        <AppLayout title="Task Categories" breadcrumbs={[{ label: 'Settings' }, { label: 'Task Categories' }]}>
            ...
            <div className="max-w-3xl">
                <div className="flex items-center justify-between mb-6">
                    <p className="text-[13px] text-[#6b7280]">Categories used to organise and filter tasks across all your projects.</p>
                ...
                {showForm && (
                    <form onSubmit={submit} className="bg-white border border-[#e5e7eb] rounded-xl p-5 mb-6">
                        ...
                        <div className="mb-4">
                            <label className="block text-[11px] tracking-[1px] uppercase text-[#6b7280] font-medium mb-2">Color</label>
                ...
                {categories.length === 0 && !showForm && (
                    <div className="text-center py-14 text-[#6b7280]">
                        ...
                        <div className="text-[13px] text-[#6b7280] mb-4">Add categories to organise and filter tasks</div>
                    </div>
                )}
                        ...
                        {categories.map(c => (
                            <div key={c.id} className="flex items-center gap-4 px-5 py-4 border-b border-[#f0f0f0] last:border-b-0 hover:bg-[#fafbfc] transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-[12px] text-[#6b7280]">{c.task_count} task{c.task_count === 1 ? '' : 's'}</div>
                                </div>
                                <button onClick={() => openEdit(c)} className="text-[#9ca3af] hover:text-[#4f6df5] transition-colors p-1.5"><Pencil size={14} /></button>
                                <button onClick={() => remove(c)} className="text-[#9ca3af] hover:text-red-500 transition-colors p-1.5"><Trash2 size={14} /></button>
                            </div>
                        ))}
                    </div>
                )}
